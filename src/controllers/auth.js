...
module.exports = {
    ...
    loginUser: async function (request, response) {
        try {
            ...
            if (!compare) {
                return helper.response(response, 401, { message: "The password you entered is incorrect" })
            }
            if (isVerify === null) {
            ...
            return helper.response(response, 401, { message: "The email you entered is incorrect" })
        }
    },
    verifyUser: async function (request, response) {
        try {
            ...
            if (!compare) {
                return helper.response(response, 401, { message: "(Verification) Invalid OTP Code" })
            } else {
            ...
            return helper.response(response, 401, { message: "Failed to verify user, account not found" })
        }
    },
    forgotPassword: async function (request, response) {
        try {
            ...
            return helper.response(response, 401, { message: "Forgot action failed, account not found" })
        }
    },
    validateUser: async function (request, response) {
        try {
            ...
            if (!compare) {
                return helper.response(response, 401, { message: "(Validation) Invalid OTP Code" })
            } else {
                const verify_code = checkUser.verify_code
                const verify = checkUser.verify
            ...
            return helper.response(response, 401, { message: "Failed to validate user, account not found" })
        }
    },
    resetPassword: async function (request, response) {
        try {
            ...
            if (reset_code === null && verify === '1') {
                ...
                return helper.response(response, 401, { message: "Please validate your account first" })
            }
        } catch (error) {
    ...
    refreshToken: async function (request, response) {
        try {
            ...
            if (result === undefined) {
                return helper.response(response, 401, { message: "Invalid email or password" })
            } else {
